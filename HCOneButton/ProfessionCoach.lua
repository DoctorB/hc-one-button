...
HCOB_Professions = HCOB_Professions or {}
local P = HCOB_Professions
...
local IT_ALIASES = {
    ["alchimia"]="ALCHEMY", ["forgiatura"]="BLACKSMITHING", ["cucina"]="COOKING",
    ["incantamento"]="ENCHANTING", ["ingegneria"]="ENGINEERING", ["primo soccorso"]="FIRSTAID",
    ["pesca"]="FISHING", ["erbalismo"]="HERBALISM", ["conciatura"]="LEATHERWORKING",
    ["estrazione"]="MINING", ["scuoiatura"]="SKINNING", ["sartoria"]="TAILORING",
}
...
local function BuildAliases()
    ...
    for name, key in pairs(IT_ALIASES) do aliases[Norm(name)] = key end
end
...
local function GenericRankHint(s)
    ...
    if s.max <= 75 and s.skill >= 50 and lvl >= 10 then return "TRAINER: impara Journeyman ora" end
    if s.max <= 150 and s.skill >= 125 and lvl >= 20 then return "TRAINER: impara Expert ora" end
    if s.max <= 225 and s.skill >= 200 and lvl >= 35 then return "TRAINER: impara Artisan ora" end
    return nil
end
...
local function FirstAidRankHint(s)
    if not s then return nil end
    local lvl = PlayerLevel()
    if s.max <= 75 and s.skill >= 50 and lvl >= 10 then return "FIRST AID: trainer Journeyman" end
    if s.max <= 150 and s.skill >= 125 then
        ...
        return "FIRST AID: compra Expert + 2 manuali da "..loc
    end
    if s.max <= 225 and s.skill >= 225 then
        if lvl < 35 then return "FIRST AID 225: porta il personaggio almeno a L35 per Artisan/Triage" end
        local loc = Faction()=="Horde" and "Doctor Gregory Victor, Hammerfall" or "Doctor Gustaf VanHowzen, Theramore"
        return "FIRST AID: completa Triage / Artisan da "..loc
    end
    return nil
end
...
local function FishingRankHint(s)
    if not s then return nil end
    local lvl = PlayerLevel()
    if s.max <= 75 and s.skill >= 50 and lvl >= 10 then return "FISHING: trainer Journeyman" end
    if s.max <= 150 and s.skill >= 125 then return "FISHING: compra Expert Fishing - The Bass and You" end
    if s.max <= 225 and s.skill >= 225 then return "FISHING: Artisan tramite quest di Nat Pagle (L35+)" end
end

local function CookingRankHint(s)
    if not s then return nil end
    local lvl = PlayerLevel()
    if s.max <= 75 and s.skill >= 50 and lvl >= 10 then return "COOKING: trainer Journeyman" end
    if s.max <= 150 and s.skill >= 125 then return "COOKING: prendi Expert Cookbook dal vendor di fazione" end
...
local FA = {
    ...
    {lo=240, hi=260, item=8545,  fallback="Heavy Mageweave Bandage",cloth=4338,  clothFallback="Mageweave Cloth", per=2, learn="Train Heavy Mageweave Bandage dal trauma surgeon"},
    {lo=260, hi=290, item=14529, fallback="Runecloth Bandage",      cloth=14047, clothFallback="Runecloth",       per=1, learn="Train Runecloth Bandage dal trauma surgeon"},
    {lo=290, hi=301, item=14530, fallback="Heavy Runecloth Bandage",cloth=14047, clothFallback="Runecloth",       per=2, learn="Train Heavy Runecloth Bandage dal trauma surgeon"},
}
...
local function FirstAidPlan(s)
    ...
    if s.skill >= 300 then return {priority=10, title="FIRST AID 300", text="Massimizzato", detail="300/300"} end
    for _,r in ipairs(FA) do
        if s.skill >= r.lo and s.skill < r.hi then
            ...
            local action = "Crafta "..recipe.." fino a "..r.hi
            if r.learn and s.skill <= (r.lo + 1) then action = r.learn.."; poi "..action end
            return {
                priority=prio, title="FIRST AID "..s.skill.."/"..s.max,
                text=action,
                detail=cloth.." "..have.." | minimo teorico "..minNeed.." / buffer ~"..buffered.." | craft ora "..can,
            }
        end
    end
end
...
local GATHER_ROUTES = {
    ...
    MINING = {
        {1,65,  "Copper Vein", "starter/secondary zones; compra e porta Mining Pick"},
...
local function GatherPlan(key, s)
    ...
    if s.skill >= 300 then return {priority=5,title=s.name.." 300",text="Massimizzato",detail="300/300"} end
...
local function FishingPlan(s)
    ...
    if s.skill >= 300 then return {priority=5,title="FISHING 300",text="Massimizzato",detail="300/300"} end
    local skill = s.skill
    local where
    if skill < 75 then where="starter zone: usa lure se molti pesci scappano"
    elseif skill < 150 then where="capitale di fazione: zona sicura, ottima anche per Cooking"
    elseif skill < 225 then where="Wetlands/Hillsbrad/Desolace: pesca mentre livelli Cooking"
    else where="Tanaris/Feralas/Azshara: pesca utile + Cooking/Alchemy" end
    local desired = min(300, PlayerLevel()*5 + 50)
    local prio = skill < desired and 500 or 180
    return {priority=prio,title="FISHING "..skill.."/"..s.max,text=where,detail="Target comfort: ~"..desired.." skill (riduce 'fish got away')"}
end
        ...
        if ok2 and name and typ and typ~="header" and typ~="subheader" and typ~="trivial" then
            ...
            for r=1,nreg do
                ...
                if (tonumber(have) or 0) < (tonumber(req) or 0) then
                    ...
                    if not missText then missText=(rn or "reagente").." "..tostring(have or 0).."/"..tostring(req or 0) end
                end
            end
...
local function OpenCraftPlan()
    ...
    if (rec.available or 0) > 0 then
        ...
        return {priority=760,title=(rec.tradeName or "PROFESSIONE").." | "..label,text="Crafta "..rec.name,detail="Disponibili "..rec.available.." | batch consigliato "..batch.." poi rivaluta colore"}
    end
    return {priority=620,title=(rec.tradeName or "PROFESSIONE").." | "..label,text="Prossimo craft efficiente: "..rec.name,detail="Manca: "..tostring(rec.missingText or "reagenti")}
end
...
local function CraftSkillPlan(key,s)
    ...
    if last then
        ...
        if (last.available or 0)>0 then
            return {priority=340,title=s.name.." "..s.skill.."/"..s.max,text="Ultimo best: "..last.name.." ("..label..")",detail="Apri la professione per ricalcolo materiali/colore"}
        end
    end
    return {priority=220,title=s.name.." "..s.skill.."/"..s.max,text="Apri la finestra professione: analizzo ricette + reagenti",detail="Preferenza: Orange > Yellow > Green; Grey mai"}
end
...
local function UpdatePanel()
    ...
    local p=plans[1]
    titleFS:SetText("PROF COACH | "..tostring(p.title or "PROFESSIONI"))
...
function P.PrintStatus()
    ...
    print("|cff55c8ffHCOB PROF COACH:|r professioni rilevate="..tostring(nskills).." (priorita ordinate)")
    if next(state.skills)==nil then print("  Nessuna professione rilevata: apri il pannello Skill/Professioni e riprova /hcob prof") return end
    for key,s in pairs(state.skills) do
