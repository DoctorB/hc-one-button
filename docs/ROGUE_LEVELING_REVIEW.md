...
- No protected macro, external input driver, saved setting or pixel slot is changed by this follow-up. Existing SS/Hemorrhage slots remain usable without an independent BASE loop; the addon cannot identify a physical mouse button remapped only in external software.
